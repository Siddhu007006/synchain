...
def _get_company_or_404(
    company_id: int, auth: AuthContext, db: Session
) -> Company:
...
async def import_csv(
    ...
    dry_run: bool = Query(True, description="true = preview only, false = execute import"),
    ...
    for r in all_preview_rows:
        if not r["valid"]:
            for err_msg in r["errors"]:
                ...
                errors.append(ImportRowError(row=r["row"], field=field, message=err_msg))
    ...
    logger.info(
        ...
        company.id, entity_type, file_name,
