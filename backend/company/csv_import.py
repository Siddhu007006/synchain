    ...
    for i, row in enumerate(rows, start=2):
        ...
        if raw_status is not None and str(raw_status).strip():
            ...
            if matched:
                ...
                errors.append(f"supply_status must be High, Medium, or Low, got '{status_str}'")
...
def _upsert_products(db: Session, company_id: int, valid_rows: list[dict]) -> tuple[int, int]:
    created, updated = 0, 0
...
def _upsert_suppliers(db: Session, company_id: int, valid_rows: list[dict]) -> tuple[int, int]:
    created, updated = 0, 0
    for data in valid_rows:
        existing = db.scalar(
            select(Supplier).where(Supplier.company_id == company_id, Supplier.name == data["name"])
        )
        if existing:
...
def _upsert_warehouses(db: Session, company_id: int, valid_rows: list[dict]) -> tuple[int, int]:
    created, updated = 0, 0
    for data in valid_rows:
        existing = db.scalar(
