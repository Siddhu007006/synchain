...
from auth.models import ROLE_MEMBER, ROLE_ADMIN
...
def create_company(
    ...
    db.commit()
    db.refresh(company)
    logger.info("Company created: id=%d name=%s (org=%d)", company.id, company.name, auth.org.id)
...
def list_companies(
    ...
    total = db.scalar(
        select(func.count()).select_from(Company).where(*base_filter)
    ) or 0
...
def delete_or_archive_company(
    ...
    warehouse_count = db.scalar(
        select(func.count()).select_from(Warehouse).where(Warehouse.company_id == company_id)
    ) or 0
    twin_count = db.scalar(
        select(func.count()).select_from(DigitalTwin).where(DigitalTwin.company_id == company_id)
    ) or 0

    # Count simulations linked to this company
    import models as sim_models
    simulation_count = db.scalar(
        select(func.count()).select_from(sim_models.Simulation).where(
            sim_models.Simulation.company_id == company_id
        )
    ) or 0

    total_dependent = product_count + supplier_count + warehouse_count + twin_count + simulation_count

    # ── BLOCK: company has data → 409 with counts ──────────────────────────
    if total_dependent > 0:
        parts = []
        if twin_count:        parts.append(f"{twin_count} twin{'s' if twin_count != 1 else ''}")
        if simulation_count:  parts.append(f"{simulation_count} simulation{'s' if simulation_count != 1 else ''}")
        if product_count:     parts.append(f"{product_count} product{'s' if product_count != 1 else ''}")
        if supplier_count:    parts.append(f"{supplier_count} supplier{'s' if supplier_count != 1 else ''}")
        if warehouse_count:   parts.append(f"{warehouse_count} warehouse{'s' if warehouse_count != 1 else ''}")

        summary = ", ".join(parts)
        logger.warning(
            "Company delete blocked: id=%d has %s (org=%d)",
            company_id, summary, auth.org.id,
        )
        raise HTTPException(
            status_code=409,
            detail={
    ...
    db.commit()
    logger.info("Company archived: id=%d name=%s (org=%d)", company_id, company.name, auth.org.id)
...
def archive_company(
    ...
    company.is_archived = True
    db.commit()
    logger.info("Company explicitly archived: id=%d name=%s (org=%d)", company_id, company.name, auth.org.id)
    return CompanyArchiveResponse(
        id=company.id,
        name=company.name,
        is_archived=True,
...
def unarchive_company(
    ...
    db.commit()
    logger.info("Company unarchived: id=%d name=%s (org=%d)", company_id, company.name, auth.org.id)
    return CompanyArchiveResponse(
        id=company.id,
        name=company.name,
        is_archived=False,
...
def list_company_twins(
    ...
    for twin in twins:
        ...
        signal_count = db.scalar(
            select(sqlfunc.count()).select_from(SignalEvent).where(
                SignalEvent.twin_id == twin.id
            )
        ) or 0
        ...
        if recent_signals:
            ...
            for i, sig in enumerate(recent_signals):
                ...
                total_weight += w
            if total_weight > 0:
                health_score = round(max(0.0, min(1.0, 1.0 - weighted_sev / total_weight)), 4)
...
def create_company_twin(
    ...
    logger.info(
        ...
        twin.id, company_id, auth.org.id,
    )
...
def create_product(
    ...
    db.refresh(product)
    logger.info(
        "Product created: id=%d name=%s company_id=%d", product.id, product.name, company_id
    )
...
def _supplier_to_resp(s: Supplier) -> SupplierResponse:
    return SupplierResponse(
        ...
        lead_time_days=s.lead_time_days, supply_status=s.supply_status,
...
def _warehouse_to_resp(w: Warehouse) -> WarehouseResponse:
    return WarehouseResponse(
        id=w.id, company_id=w.company_id, name=w.name,
        warehouse_id=w.warehouse_id, location=w.location, capacity=w.capacity,
        created_at=w.created_at.isoformat() if w.created_at else None,
        updated_at=w.updated_at.isoformat() if w.updated_at else None,
    )


@router.post("/{company_id}/suppliers", response_model=SupplierResponse,
             status_code=status.HTTP_201_CREATED,
             dependencies=[Depends(require_role(ROLE_MEMBER))], tags=["Suppliers"])
def create_supplier(company_id: int, payload: SupplierCreateRequest,
                    auth: AuthContext = Depends(get_current_user),
                    db: Session = Depends(get_db)):
    """Create a supplier.  lead_time_days → supplier_delay,  supply_status → supply_status."""
    _verify_company(company_id, auth.org.id, db)
    s = Supplier(company_id=company_id, name=payload.name,
...
@router.get("/{company_id}/suppliers", response_model=SupplierListResponse, tags=["Suppliers"])
def list_company_suppliers(company_id: int, limit: int = 100, offset: int = 0,
                           auth: AuthContext = Depends(get_current_user),
                           db: Session = Depends(get_db)):
    """List all suppliers for a company (V2.5 prefill ready)."""
    _verify_company(company_id, auth.org.id, db)
    total = db.scalar(select(func.count()).select_from(Supplier)
                      .where(Supplier.company_id == company_id)) or 0
    rows = db.scalars(select(Supplier).where(Supplier.company_id == company_id)
                      .order_by(Supplier.name).limit(limit).offset(offset)).all()
    return SupplierListResponse(total=total, suppliers=[_supplier_to_resp(s) for s in rows])


@router.get("/{company_id}/suppliers/{supplier_id}",
            response_model=SupplierResponse, tags=["Suppliers"])
def get_supplier(company_id: int, supplier_id: int,
                 auth: AuthContext = Depends(get_current_user),
                 db: Session = Depends(get_db)):
    _verify_company(company_id, auth.org.id, db)
    s = db.scalar(select(Supplier).where(Supplier.id == supplier_id,
                                         Supplier.company_id == company_id))
    if not s: raise NotFoundError(f"Supplier {supplier_id} not found")
    return _supplier_to_resp(s)


@router.patch("/{company_id}/suppliers/{supplier_id}",
              response_model=SupplierResponse,
              dependencies=[Depends(require_role(ROLE_MEMBER))], tags=["Suppliers"])
def update_supplier(company_id: int, supplier_id: int, payload: SupplierUpdateRequest,
                    auth: AuthContext = Depends(get_current_user),
                    db: Session = Depends(get_db)):
    _verify_company(company_id, auth.org.id, db)
    s = db.scalar(select(Supplier).where(Supplier.id == supplier_id,
    ...
    if payload.lead_time_days is not None: s.lead_time_days = payload.lead_time_days
    if payload.supply_status is not None: s.supply_status = payload.supply_status
    if payload.reliability_pct is not None: s.reliability_pct = payload.reliability_pct
    db.commit(); db.refresh(s)
    return _supplier_to_resp(s)


@router.delete("/{company_id}/suppliers/{supplier_id}",
               status_code=status.HTTP_204_NO_CONTENT,
               dependencies=[Depends(require_role(ROLE_MEMBER))], tags=["Suppliers"])
def delete_supplier(company_id: int, supplier_id: int,
                    auth: AuthContext = Depends(get_current_user),
                    db: Session = Depends(get_db)):
    _verify_company(company_id, auth.org.id, db)
    s = db.scalar(select(Supplier).where(Supplier.id == supplier_id,
...
@router.post("/{company_id}/warehouses", response_model=WarehouseResponse,
             status_code=status.HTTP_201_CREATED,
             dependencies=[Depends(require_role(ROLE_MEMBER))], tags=["Warehouses"])
def create_warehouse(company_id: int, payload: WarehouseCreateRequest,
                     auth: AuthContext = Depends(get_current_user),
                     db: Session = Depends(get_db)):
    """Create a warehouse.  warehouse_id (W1/W2/W3) → SimulationInput.warehouse."""
    _verify_company(company_id, auth.org.id, db)
    w = Warehouse(company_id=company_id, name=payload.name,
                  warehouse_id=payload.warehouse_id,
                  location=payload.location, capacity=payload.capacity)
    db.add(w); db.commit(); db.refresh(w)
    logger.info("Warehouse created: id=%d company_id=%d wh=%s", w.id, company_id, w.warehouse_id)
    return _warehouse_to_resp(w)


@router.get("/{company_id}/warehouses", response_model=WarehouseListResponse, tags=["Warehouses"])
def list_company_warehouses(company_id: int, limit: int = 100, offset: int = 0,
                            auth: AuthContext = Depends(get_current_user),
                            db: Session = Depends(get_db)):
    """List all warehouses for a company (V2.5 prefill ready)."""
    _verify_company(company_id, auth.org.id, db)
    total = db.scalar(select(func.count()).select_from(Warehouse)
                      .where(Warehouse.company_id == company_id)) or 0
    rows = db.scalars(select(Warehouse).where(Warehouse.company_id == company_id)
                      .order_by(Warehouse.name).limit(limit).offset(offset)).all()
    return WarehouseListResponse(total=total, warehouses=[_warehouse_to_resp(w) for w in rows])


@router.get("/{company_id}/warehouses/{warehouse_db_id}",
            response_model=WarehouseResponse, tags=["Warehouses"])
def get_warehouse(company_id: int, warehouse_db_id: int,
                  auth: AuthContext = Depends(get_current_user),
                  db: Session = Depends(get_db)):
    _verify_company(company_id, auth.org.id, db)
    w = db.scalar(select(Warehouse).where(Warehouse.id == warehouse_db_id,
                                          Warehouse.company_id == company_id))
    if not w: raise NotFoundError(f"Warehouse {warehouse_db_id} not found")
    return _warehouse_to_resp(w)


@router.patch("/{company_id}/warehouses/{warehouse_db_id}",
              response_model=WarehouseResponse,
              dependencies=[Depends(require_role(ROLE_MEMBER))], tags=["Warehouses"])
def update_warehouse(company_id: int, warehouse_db_id: int, payload: WarehouseUpdateRequest,
                     auth: AuthContext = Depends(get_current_user),
                     db: Session = Depends(get_db)):
    _verify_company(company_id, auth.org.id, db)
    w = db.scalar(select(Warehouse).where(Warehouse.id == warehouse_db_id,
    ...
    if payload.warehouse_id is not None: w.warehouse_id = payload.warehouse_id
    if payload.location is not None: w.location = payload.location
    if payload.capacity is not None: w.capacity = payload.capacity
    db.commit(); db.refresh(w)
    return _warehouse_to_resp(w)


@router.delete("/{company_id}/warehouses/{warehouse_db_id}",
               status_code=status.HTTP_204_NO_CONTENT,
               dependencies=[Depends(require_role(ROLE_MEMBER))], tags=["Warehouses"])
def delete_warehouse(company_id: int, warehouse_db_id: int,
                     auth: AuthContext = Depends(get_current_user),
                     db: Session = Depends(get_db)):
    _verify_company(company_id, auth.org.id, db)
    w = db.scalar(select(Warehouse).where(Warehouse.id == warehouse_db_id,
