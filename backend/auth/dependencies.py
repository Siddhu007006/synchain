...
from auth.models import (
    User, Organization, Membership, APIKey,
    ROLE_HIERARCHY, VALID_ROLES, ROLE_OWNER,
)
...
async def get_current_user(
    ...
    # SECURITY: DEBUG bypass restored for local frontend / run_full_audit compatibility
    if credentials is None:
        if settings.debug:
            # Check if default developer user exists
            dev_email = "debug@test.local"
            user = db.execute(select(User).where(User.email == dev_email)).scalar_one_or_none()
            if user is None:
                # Create user
                user = User(
                    email=dev_email,
                    hashed_password="debug-password-not-used",
                ...
                if membership is None:
                    ...
                    db.commit()
                    db.refresh(membership)
                org = db.execute(select(Organization).where(Organization.id == membership.org_id)).scalar_one()

            # Set user context for structured logging
            try:
                from logging_config import user_id_var, org_id_var
                user_id_var.set(user.id)
                org_id_var.set(org.id)
            except ImportError:
                pass

            # Expose user_id on request.state
    ...
    # --- Try API key ---
        ...
        api_key_hash = __import__("auth.security", fromlist=["hash_api_key"]).hash_api_key(token)
    ...
    if token.startswith("sc_live_"):
        ...
        api_key_hash_val = __import__("auth.security", fromlist=["hash_api_key"]).hash_api_key(token)
        ak = db.execute(
