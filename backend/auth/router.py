...
from auth.models import User, Organization, Membership, APIKey, ROLE_OWNER, ROLE_ADMIN
...
async def get_me(auth: AuthContext = Depends(get_current_user), db: Session = Depends(get_db)):
    """Return the current user's profile with all org memberships."""
    memberships = db.execute(
        select(Membership).where(Membership.user_id == auth.user.id)
    ).scalars().all()
    ...
    for m in memberships:
        ...
        if org:
            orgs_summary.append(UserOrgSummary(
                org_id=org.id,
                org_name=org.name,
                org_slug=org.slug,
                role=m.role,
...
async def create_api_key_endpoint(
    ...
    logger.info("API key created: %s (user: %s, org: %s)", key_prefix, auth.user.email, auth.org.slug)
