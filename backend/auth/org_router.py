...
def _verify_membership(db: Session, user_id: int, org_id: int, min_role: str) -> Membership:
...
async def list_orgs(
    ...
    for m in memberships:
        org = db.execute(select(Organization).where(Organization.id == m.org_id)).scalar_one_or_none()
        if org:
            result.append(OrgListItem(
                id=org.id, name=org.name, slug=org.slug,
                role=m.role, member_count=_get_member_count(db, org.id),
            ))
    return result
...
async def get_org(
    ...
    return OrgResponse(
        id=org.id, name=org.name, slug=org.slug,
        created_at=org.created_at, member_count=_get_member_count(db, org.id),
    )
