...
class TestOrgCRUD:
    ...
    def test_create_duplicate_org_name(self, client):
        ...
        resp = client.post("/api/v1/orgs/", json={"name": "Unique Corp"}, headers=headers)
...
class TestOrgPermissions:
    def test_viewer_cannot_invite(self, client):
        admin = _register_user(client, email="orgadmin@synchain.io", org_name="Perm Org")
        viewer = _register_user(client, email="orgviewer@synchain.io", org_name="Viewer Org")
        ...
        third = _register_user(client, email="third@synchain.io", org_name="Third")
        resp = client.post(
            f"/api/v1/orgs/{admin['org_slug']}/members",
