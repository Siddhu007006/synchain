...
class TestSignalPenalty:
    ...
    def test_trend_shift_no_penalty(self):
        ...
        penalty, details = compute_signal_penalty([
            _sig("TrendShift", 0.8, {"old_trend": "Stable", "new_trend": "Rising", "shift_type": "acceleration"}),
        ])
        assert penalty == 0.0
        assert details == []
...
class TestSignalExplanation:
    ...
    def test_trend_shift_note(self):
        ...
        result = build_signal_explanation(
            ...
            elevating_source=None,
            elevating_severity=None,
            active_signals=[_sig("TrendShift", 0.3, {
                "old_trend": "Stable", "new_trend": "Rising", "shift_type": "acceleration",
            })],
        )
...
class TestApplySignalAdjustments:
    ...
    def test_demand_spike_only(self):
        adj = apply_signal_adjustments(
            0.80, "Low", [_sig("DemandSpike", 0.5)]
        )
    ...
    def test_supplier_degradation_with_elevation(self):
        adj = apply_signal_adjustments(
            0.70, "Low", [_sig("SupplierDegradation", 0.8)]
        )
...
class TestE4Integration:
    ...
    def _create_twin_with_spike(self, client, h, product="SpikeProd"):
        ...
        client.post("/api/v1/simulate", json={
            "product": product,
            "stock": 5000, "warehouse": "W1", "demand": 5000,
            "supplier_delay": 2, "twin_id": twin_id,
        }, headers=h)
        # Second sim: spike demand (triggers DemandSpike)
        client.post("/api/v1/simulate", json={
            "product": product,
            "stock": 5000, "warehouse": "W1", "demand": 15000,
            "supplier_delay": 2, "twin_id": twin_id,
        }, headers=h)
        return twin_id
    ...
    def _create_twin_with_degradation(self, client, h, product="DegProd"):
        ...
        twin_resp = client.post("/api/v1/twins", json={"name": "E4 Deg Test"}, headers=h)
        twin_id = twin_resp.json()["id"]
        ...
        for _ in range(3):
            client.post("/api/v1/simulate", json={
                "product": product,
                "stock": 5000, "warehouse": "W1", "demand": 5000,
                "supplier_delay": 9, "supply_status": "Low",
                "twin_id": twin_id,
            }, headers=h)
        return twin_id
    ...
    def test_forecast_confidence_reduced_by_spike(self, auth_client):
        ...
        if has_spike:
            ...
            assert "Signal adjustments:" in fc["explanation"] or "DemandSpike" in fc["explanation"]
    ...
    def test_clean_twin_matches_e2(self, auth_client):
        ...
        expected = resp.json()["source_state"]["avg_demand"] * fc["trend_factor"] * fc["season_factor"]
