...
class TestSyntheticProviders:
    ...
    def test_provider_source_names(self):
        from signals.providers import (
            SyntheticNewsProvider, SyntheticWeatherProvider,
            SyntheticCommodityProvider, SyntheticEconomicProvider,
        )
...
class TestNewsDetector:
    ...
    def _populate_cache(self, db, events):
        from signals.external_cache import CacheManager
        cm = CacheManager(db)
        cm.upsert("news_synthetic", "global", {
            "events": events, "fetched_at": int(time.time()), "provider": "news_synthetic"
        })
        db.commit()

    def test_disruption_triggers(self, e5_db):
        from signals.external_detectors import NewsDisruptionDetector
        self._populate_cache(e5_db, [
            {"headline": "Port strike", "category": "logistics", "relevance_score": 70, "source": "test"}
        ])
    ...
    def test_low_relevance_no_trigger(self, e5_db):
        from signals.external_detectors import NewsDisruptionDetector
        self._populate_cache(e5_db, [
            {"headline": "Normal news", "category": "general", "relevance_score": 20, "source": "test"}
        ])
    ...
    def test_multiple_events(self, e5_db):
        from signals.external_detectors import NewsDisruptionDetector
        self._populate_cache(e5_db, [
            {"headline": "Strike", "category": "logistics", "relevance_score": 60, "source": "t"},
            {"headline": "Fire", "category": "supply", "relevance_score": 80, "source": "t"},
            {"headline": "Peace", "category": "general", "relevance_score": 10, "source": "t"},
        ])
        signals = NewsDisruptionDetector().evaluate(_mock_twin(), e5_db)
...
class TestWeatherDetector:
    ...
    def _populate_cache(self, db, condition, severity_level, base_severity=0.0):
        from signals.external_cache import CacheManager
        cm = CacheManager(db)
        cm.upsert("weather_synthetic", "global", {
            "condition": condition,
...
class TestCommodityDetector:
    ...
    def _populate_cache(self, db, commodities):
        from signals.external_cache import CacheManager
        cm = CacheManager(db)
        cm.upsert("commodity_synthetic", "global", {
            "commodities": commodities, "fetched_at": int(time.time()), "provider": "commodity_synthetic"
        })
        db.commit()
    ...
    def test_spike_triggers(self, e5_db):
        ...
        self._populate_cache(e5_db, [
            {"commodity": "Steel", "baseline_price": 800, "current_price": 960, "change_pct": 20.0}
        ])
    ...
    def test_stable_no_trigger(self, e5_db):
        from signals.external_detectors import CommodityShockDetector
        self._populate_cache(e5_db, [
            {"commodity": "Steel", "baseline_price": 800, "current_price": 840, "change_pct": 5.0}
        ])
        signals = CommodityShockDetector().evaluate(_mock_twin(), e5_db)
        assert len(signals) == 0
    ...
    def test_max_severity_capped(self, e5_db):
        from signals.external_detectors import CommodityShockDetector
        self._populate_cache(e5_db, [
            {"commodity": "Oil", "baseline_price": 75, "current_price": 150, "change_pct": 100.0}
        ])
        signals = CommodityShockDetector().evaluate(_mock_twin(), e5_db)
        assert len(signals) == 1
        assert signals[0].severity == 1.0
...
class TestEconomicDetector:
    ...
    def _populate_cache(self, db, pmi, inflation):
        from signals.external_cache import CacheManager
        cm = CacheManager(db)
        cm.upsert("economic_synthetic", "global", {
            "indicators": {"pmi": pmi, "inflation_pct": inflation, "consumer_confidence": 100},
        })
        db.commit()
...
class TestDetectorRegistry:
    ...
    def test_external_detectors_present(self):
        ...
        from signals.external_detectors import (
            NewsDisruptionDetector, WeatherAlertDetector,
            CommodityShockDetector, EconomicShiftDetector,
        )
        detector_types = [type(d) if not isinstance(d, type) else d for d in ALL_DETECTORS]
