...
class TestValidateProducts:
    def test_valid_rows(self):
        rows = [
            {"name": "Laptop", "category": "Electronics", "stock": "500", "demand": "700"},
            {"name": "Mouse", "category": "Peripherals", "stock": "1000", "demand": "200"},
        ]
...
class TestValidateSuppliers:
    def test_valid_supplier(self):
        rows = [{"name": "Supplier A", "lead_time": "8", "reliability": "72", "status": "Low"}]
...
class TestValidateWarehouses:
    def test_valid_warehouse(self):
        rows = [{"name": "Hub", "location": "Mumbai", "capacity": "10000", "warehouse_id": "W1"}]
...
class TestUpsertProducts:
    def test_insert_new(self, db_session):
        ...
        rows = [
            {"name": "Laptop", "category": "Electronics", "current_stock": 500.0, "avg_monthly_demand": 700.0},
            {"name": "Mouse", "category": "Peripherals", "current_stock": 1000.0, "avg_monthly_demand": 200.0},
        ]
        created, updated = upsert_products(session, company_id, rows)
        assert created == 2
        assert updated == 0

        products = session.scalars(select(Product).where(Product.company_id == company_id)).all()
        assert len(products) == 2

    def test_update_existing(self, db_session):
        session, company_id = db_session
        # Insert first
        session.add(Product(company_id=company_id, name="Laptop", current_stock=100, avg_monthly_demand=200))
        session.flush()

        # Upsert with updated values
        rows = [{"name": "Laptop", "category": "Electronics", "current_stock": 500.0, "avg_monthly_demand": 700.0}]
        created, updated = upsert_products(session, company_id, rows)
        assert created == 0
        assert updated == 1
    ...
    def test_mixed_insert_update(self, db_session):
        ...
        rows = [
            ...
            {"name": "Tablet", "category": "Electronics", "current_stock": 50.0, "avg_monthly_demand": 30.0},
        ]
        created, updated = upsert_products(session, company_id, rows)
        assert created == 1
        assert updated == 1
...
class TestUpsertSuppliers:
    def test_insert_and_update(self, db_session):
        session, company_id = db_session
        session.add(Supplier(company_id=company_id, name="Supplier A", lead_time_days=5))
        session.flush()

        rows = [
            {"name": "Supplier A", "lead_time_days": 10.0, "supply_status": "Low", "reliability_pct": 60.0},
            {"name": "Supplier B", "lead_time_days": 3.0, "supply_status": "High", "reliability_pct": 95.0},
        ]
...
class TestUpsertWarehouses:
    def test_insert_and_update(self, db_session):
        ...
        rows = [
            {"name": "Hub A", "warehouse_id": "W2", "location": "Delhi", "capacity": 5000.0},
            {"name": "Hub B", "warehouse_id": "W3", "location": "Chennai", "capacity": 8000.0},
        ]
...
class TestFullPipeline:
    ...
    def test_upsert_pipeline(self, db_session):
        ...
        total = session.scalars(select(Product).where(Product.company_id == company_id)).all()
        assert len(total) == 1
