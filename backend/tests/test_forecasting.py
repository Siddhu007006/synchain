...
class TestExplanation:
    ...
    def test_contains_base_demand(self):
        text = build_explanation(
            "Widget-A", 1, 8000.0, 9660.0, 1.05, "Rising",
            1.15, "Festival", "Medium", 65.0, 0.70, 7,
        )
    ...
    def test_contains_supply_risk(self):
        text = build_explanation(
            "Widget-A", 1, 8000.0, 9660.0, 1.05, "Rising",
            1.15, "Festival", "High", 30.0, 0.50, 3,
        )
...
class TestForecastEngine:
    ...
    def _create_twin_with_sim(self, client, h, product="Widget-A", demand=8000):
        ...
        twin_resp = client.post("/api/v1/twins", json={"name": "Forecast Test"}, headers=h)
...
class TestForecastAPI:
    ...
    def _create_twin_with_sim(self, client, h, product="Widget-X", demand=8000):
        twin_resp = client.post("/api/v1/twins", json={"name": "API Forecast"}, headers=h)
        twin_id = twin_resp.json()["id"]
        client.post("/api/v1/simulate", json={
            "product": product,
            "stock": 5000,
    ...
    def test_demand_not_contaminated_by_supply(self, auth_client):
        ...
        expected = body["source_state"]["avg_demand"] * fc["trend_factor"] * fc["season_factor"]
    ...
    def test_list_records_filter_by_product(self, auth_client):
        ...
        client.get(f"/api/v1/twins/{twin_id}/forecast", params={"product": "Widget-A"}, headers=h)
        client.get(f"/api/v1/twins/{twin_id}/forecast", params={"product": "Widget-B"}, headers=h)
        resp = client.get(
