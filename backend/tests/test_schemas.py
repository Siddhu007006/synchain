...
class TestSimulationInputValidation:
    ...
    def test_invalid_warehouse(self):
        with pytest.raises(ValidationError, match="W1.*W2.*W3"):
            SimulationInput(
                product="Test", stock=100, warehouse="W4",
                demand=200, supplier_delay=3,
            )

    def test_invalid_market_trend(self):
        with pytest.raises(ValidationError):
            SimulationInput(
                product="Test", stock=100, warehouse="W1",
                demand=200, supplier_delay=3, market_trend="Bullish",
            )
    ...
    def test_invalid_supply_status(self):
        with pytest.raises(ValidationError):
            SimulationInput(
                product="Test", stock=100, warehouse="W1",
                demand=200, supplier_delay=3, supply_status="None",
            )
    ...
    def test_invalid_season(self):
        with pytest.raises(ValidationError):
            SimulationInput(
                product="Test", stock=100, warehouse="W1",
                demand=200, supplier_delay=3, season="Summer",
            )
            ...
            for trend in ["Positive", "Neutral", "Negative"]:
                for supply in ["High", "Medium", "Low"]:
                    for season in ["Festival", "Normal", "Off-season"]:
                        si = SimulationInput(
                            product="Test", stock=100, warehouse=wh,
                            demand=200, supplier_delay=3,
                            market_trend=trend, supply_status=supply,
                            season=season,
                        )
