...
RISK_ELEVATION_SOURCES = {
    # E4 atomic
    "SupplierDegradation", "WarehouseOverload", "WeatherAlert",
    # E6 compound
    "SupplyShock", "FulfillmentCrisis", "PerfectStorm",
}
...
def compute_signal_penalty(active_signals: list) -> tuple[float, list[str]]:
    ...
    for sig in active_signals:
        ...
        if penalty > 0:
            total += penalty
            details.append(f"{source} penalty: -{penalty:.2f} (severity {severity:.2f})")

    return round(total, 4), details


def compute_risk_elevation(base_risk: str, active_signals: list) -> tuple[str, bool, str | None]:
    """
...
def build_explanation(
    ...
    parts.append(
        f"Supply risk: {supply_risk} (reliability {reliability:.1f}%)."
    )
    parts.append(
...
class ForecastEngine:
    ...
    def generate(
        ...
        inputs = ForecastInput(
            ...
            season_mode=twin.market_state.season_mode if twin.market_state else "Normal",
            supplier_reliability=(
                twin.supplier_state.reliability_score
        ...
        try:
            ...
            active_signals = sig_engine.get_active_signals_for_product(
                twin_id=twin_id, product=product, limit=10,
            )
        except Exception:
        ...
        for h in horizons:
            point = generate_forecast_point(
                inputs, h, active_signals=active_signals or None,
            )
