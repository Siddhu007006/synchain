...
class CompoundRule:
    ...
    def compute_severity(self, atomic_signals: dict[str, float]) -> float:
        ...
        """
        trigger_severities = [
            atomic_signals[t] for t in self.triggers
            if t in atomic_signals
        ]
    ...
    def evaluate(self, atomic_outputs: list[SignalOutput]) -> list[SignalOutput]:
        ...
        for rule in COMPOUND_RULES:
            if rule.matches(atomic_map):
                ...
                trigger_severities = {
                    t: round(atomic_map.get(t, 0.0), 4)
                    for t in rule.triggers
                }

                compounds.append(SignalOutput(
                ...
                logger.info(
                    ...
                    rule.output_source, severity, trigger_severities,
                )
