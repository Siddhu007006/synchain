...
class SupplierDegradationDetector(SignalDetector):
    ...
    def evaluate(self, twin: DigitalTwin, db: Session) -> list[SignalOutput]:
        ...
        severity = min(
            1.0,
            round((SUPPLIER_DEGRADATION_THRESHOLD - ss.reliability_score)
                  / SUPPLIER_DEGRADATION_THRESHOLD, 4),
        )
        return [SignalOutput(
            source=self.source,
            signal_type=self.signal_type,
    ...
    def evaluate(self, twin: DigitalTwin, db: Session) -> list[SignalOutput]:
        ...
        for ws in twin.warehouse_states:
            if ws.utilization_pct > WAREHOUSE_OVERLOAD_THRESHOLD:
                severity = min(
                    1.0,
                    round((ws.utilization_pct - WAREHOUSE_OVERLOAD_THRESHOLD)
                          / (1.0 - WAREHOUSE_OVERLOAD_THRESHOLD), 4),
                )
                signals.append(SignalOutput(
                    source=self.source,
                    signal_type=self.signal_type,
