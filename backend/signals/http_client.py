...
class RateLimitedClient:
    ...
    def get(
        ...
        for attempt in range(MAX_RETRIES):
            ...
            if wait > 0:
                logger.info(
                    "Rate limit: waiting %.1fs for %s", wait, domain,
                )
                time.sleep(wait)
            ...
            try:
                ...
                if response.status_code in RETRYABLE_STATUS:
                    ...
                    logger.warning(
                        ...
                        response.status_code, domain, backoff,
                ...
                # Non-retryable error
                logger.warning(
                    "HTTP %d from %s (non-retryable)", response.status_code, domain,
                )
                return None
