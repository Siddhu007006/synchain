...
def _estimate_news_relevance(title: str) -> float:
    ...
    keywords = [
        ...
        "port", "logistics", "tariff", "strike", "factory",
        "manufacturing", "freight", "shipping", "embargo",
        "sanctions", "recall", "fire", "flood", "earthquake",
    ]
...
_COMMODITY_SYMBOLS = [
    ...
    {"symbol": "COPPER", "function": "COPPER", "name": "Copper", "baseline_price": 9000.0},
    {"symbol": "ALUMINUM", "function": "ALUMINUM", "name": "Aluminum", "baseline_price": 2400.0},
]
    ...
    def fetch(self, context: dict) -> dict:
        ...
        for spec in _COMMODITY_SYMBOLS:
            ...
            if not data or "data" not in data:
                logger.warning(
                    "Alpha Vantage returned no data for %s", spec["name"],
                )
            ...
            if entries:
                ...
                change_pct = round(
                    ((current_price - spec["baseline_price"]) / spec["baseline_price"]) * 100,
                    2,
                )
                commodities.append({
                    "commodity": spec["name"],
