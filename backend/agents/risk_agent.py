...
from agents.base_agent import BaseAgent, AgentStepResult
...
def _factor_direction(score: float, thresholds: tuple[float, float] = (0.6, 0.4)) -> str:
...
class RiskAgent(BaseAgent):
    ...
    def run(
        ...
        elif len([d for d in directions if d == max(set(directions), key=directions.count)]) >= 2:
        ...
        confidence = round(min(base_confidence + factor_agreement + boundary_distance, 1.0), 2)
