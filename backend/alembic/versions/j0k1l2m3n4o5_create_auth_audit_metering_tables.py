...
def upgrade() -> None:
    ...
    if not _table_exists('usage_events'):
        ...
        op.create_index('ix_usage_org_timestamp', 'usage_events', ['org_id', 'timestamp'])


def downgrade() -> None:
    """Drop auth, audit, and metering tables."""
    for table in ['usage_events', 'audit_logs_archive', 'audit_logs',
                  'api_keys', 'memberships', 'organizations', 'users']:
