...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('supply_status', sa.String(), nullable=False, server_default='Medium'),
    ...
    with op.batch_alter_table('simulations', schema=None) as batch_op:
        ...
        batch_op.add_column(sa.Column('warehouse_record_id', sa.Integer(), nullable=True))
        batch_op.create_index('ix_simulations_supplier_id', ['supplier_id'], unique=False)
        batch_op.create_index('ix_simulations_warehouse_record_id',
                              ['warehouse_record_id'], unique=False)
        batch_op.create_foreign_key(
            'fk_simulations_supplier_id', 'suppliers', ['supplier_id'], ['id']
        )
        batch_op.create_foreign_key(
            'fk_simulations_warehouse_record_id', 'warehouses',
            ['warehouse_record_id'], ['id']
        )


def downgrade() -> None:
    with op.batch_alter_table('simulations', schema=None) as batch_op:
        batch_op.drop_constraint('fk_simulations_warehouse_record_id', type_='foreignkey')
