...
from exceptions import SynChainError, SimulationError, NotFoundError, ValidationError
...
from signals.scheduler import refresh_all_providers, run_scheduler, DEFAULT_REFRESH_HOURS, DEFAULT_PROVIDER_MODE, DEFAULT_CACHE_TTL_HOURS
...
from middleware import RequestIDMiddleware, TimingMiddleware, SecurityHeadersMiddleware
...
# ---------------------------------------------------------------------------
# Lifespan: background scheduler for external data refresh
# ---------------------------------------------------------------------------
import asyncio
...
# Production hardening: restrict accepted Host headers.
# Only enforced when an explicit allowlist is configured. The wildcard
# default (["*"]) disables enforcement and accepts any Host.
...
def simulate(
    ...
    # E9: Audit + Metering
    AuditService(db).log(
        action="simulation.create", resource_type="Simulation",
        resource_id=sim.id, user_id=auth.user.id, org_id=auth.org.id,
        details={"product": payload.product, "warehouse": payload.warehouse},
    )
    MeteringService(db).record(
        org_id=auth.org.id, event_type="simulation.run",
...
def list_simulations(
    ...
    query = (
        ...
        .filter(models.Simulation.org_id == auth.org.id)
    )
    ...
    return twin_schemas.TwinDetailResponse(
        ...
        supplier_state=(
            twin_schemas.SupplierStateSnapshot(
                ...
                updated_at=twin.supplier_state.updated_at.isoformat()
        ...
        market_state=(
            twin_schemas.MarketStateSnapshot(
                ...
                updated_at=twin.market_state.updated_at.isoformat()
...
def generate_forecast(
    ...
    try:
        ...
        raise ValidationError("horizons must be comma-separated integers (e.g. '1,3,5')")
    ...
    for sig in result.get("active_signals", []):
        payload = sig.payload if isinstance(sig.payload, dict) else _json.loads(sig.payload or "{}")
    ...
    return forecast_schemas.ForecastResponse(
        ...
        source_state=forecast_schemas.ForecastSourceState(
            **result["source_state"]
        ),
        forecasts=[
    ...
    return signal_schemas.SignalListResponse(
        ...
        signals=[
            signal_schemas.SignalEventEntry(
                ...
                payload=_json.loads(s.payload) if isinstance(s.payload, str) else s.payload,
...
def get_signal_summary(
    ...
    if summary["latest_critical"]:
        ...
        latest_critical_entry = signal_schemas.SignalEventEntry(
            ...
            payload=_json.loads(lc.payload) if isinstance(lc.payload, str) else lc.payload,
...
def get_external_status(db: Session = Depends(get_db)):
    ...
    for e in entries:
        ...
        for cat, info in provider_info.items():
            if info["active_provider"] == provider_name or f"{cat}_synthetic" == provider_name:
                category = cat
                break
        ...
        providers[provider_name] = {
            ...
            "api_key_configured": provider_info.get(category, {}).get("configured", False) if category else False,
...
def trigger_external_refresh(
    ...
    results = refresh_all_providers(
        db, mode=mode, ttl_hours=DEFAULT_CACHE_TTL_HOURS,
    )
    return {
        "refreshed": True,
        "mode_used": mode,
...
@legacy.post("/simulate", response_model=schemas.SimulationCreateResponse,
...
@legacy.get("/simulate/{simulation_id}", response_model=schemas.SimulationDetailResponse)
...
@legacy.get("/simulate/{simulation_id}/scenarios", response_model=schemas.ScenarioResponse)
