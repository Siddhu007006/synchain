...
class TwinManager:
    ...
    def update_state_from_simulation(
        ...
        try:
            ...
            logging.getLogger("synchain.signals").exception(
                "Signal evaluation failed for twin %d (non-blocking)", twin_id,
            )
    ...
    def _update_product_state(
        ...
        self._log_change(twin.id, "product", product_name, "latest_stock",
                         product_state.latest_stock, stock)
        self._log_change(twin.id, "product", product_name, "latest_demand",
                         product_state.latest_demand, demand)
        self._log_change(twin.id, "product", product_name, "avg_demand",
                         old_avg, new_avg)

        old_trend = product_state.demand_trend
        new_trend = classify_trend(demand, new_avg)
        if old_trend != new_trend:
            self._log_change(twin.id, "product", product_name, "demand_trend",
                             old_trend, new_trend)
    ...
    def _update_warehouse_state(self, twin: DigitalTwin, sim_result: dict) -> None:
        ...
        for ws in twin.warehouse_states:
            ...
            if old_rate != ws.selection_rate:
                self._log_change(twin.id, "warehouse", ws.warehouse_id,
                                 "selection_rate", old_rate, ws.selection_rate)
    ...
    def _update_supplier_state(self, twin: DigitalTwin, sim_input: dict) -> None:
        ...
        self._log_change(twin.id, "supplier", "aggregate", "avg_delay",
                         old_delay, ss.avg_delay)
        ...
        if delay > ss.max_delay_seen:
            ...
            self._log_change(twin.id, "supplier", "aggregate", "max_delay_seen",
                             old_max, delay)

        # Recompute reliability
        old_reliability = ss.reliability_score
        ss.reliability_score = compute_reliability_score(ss.avg_delay, ss.max_delay_seen)
        if old_reliability != ss.reliability_score:
            self._log_change(twin.id, "supplier", "aggregate", "reliability_score",
                             old_reliability, ss.reliability_score)
    ...
    def _update_market_state(
        ...
        if ms.trend_mode != market_trend:
            self._log_change(twin.id, "market", "global", "trend_mode",
                             ms.trend_mode, market_trend)
            ms.trend_mode = market_trend

        if ms.season_mode != season:
            self._log_change(twin.id, "market", "global", "season_mode",
        ...
        self._log_change(twin.id, "market", "global", "avg_confidence",
                         old_conf, ms.avg_confidence)

        # EWMA on risk
        old_risk = ms.avg_risk_score
        ms.avg_risk_score = ewma(risk_score, old_risk)
        self._log_change(twin.id, "market", "global", "avg_risk_score",
                         old_risk, ms.avg_risk_score)
