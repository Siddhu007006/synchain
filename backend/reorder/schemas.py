...
class ReorderRecommendation(BaseModel):
    ...
    daily_demand: float = Field(
        ..., description="forecast_demand / 30"
    )
    ...
    stockout_date: str = Field(
        ..., description="Projected stockout date (ISO format)"
    )

    # Reorder Point calculation
    reorder_point: float = Field(
        ..., description="Lead Time Demand + Safety Stock"
    )
    safety_stock: float
...
class InventoryHealthResponse(BaseModel):
    ...
    score: int = Field(
        ..., ge=0, le=100,
        description="Inventory health score (0-100)"
    )
    grade: str = Field(
        ..., description="HEALTHY | MODERATE | AT_RISK | CRITICAL | N/A"
    )
