...
SUPPLIERS = {
    "Acme Components": SupplierData(1, "Acme Components", lead_time_days=14, reliability_pct=92, supply_status="High"),
    "FastParts Inc": SupplierData(2, "FastParts Inc", lead_time_days=7, reliability_pct=88, supply_status="High"),
    "GlobalTech Supply": SupplierData(3, "GlobalTech Supply", lead_time_days=21, reliability_pct=78, supply_status="Medium"),
    "BudgetBuild Co": SupplierData(4, "BudgetBuild Co", lead_time_days=10, reliability_pct=65, supply_status="Low"),
    "PrecisionMfg Ltd": SupplierData(5, "PrecisionMfg Ltd", lead_time_days=18, reliability_pct=96, supply_status="High"),
}
...
def run_audit_1():
    ...
    for i, (name, stock, demand, price, supplier_key, fc_demand, fc_conf, sig_crit) in enumerate(PRODUCTS, 1):
        ...
        print(f"│  Stock: {stock:,.0f}  │  Demand: {fc_demand or demand}/mo  │  Price: ${price:.2f}")
        print(f"│  Supplier: {supplier_key} (LT: {supplier.lead_time_days}d, Rel: {supplier.reliability_pct}%)")
        print("│")
        ...
        print(f"│  ▸ Reorder Point:   {result.reorder_point:,.1f} (LTD: {result.lead_time_demand:.1f} + SS: {result.safety_stock:.1f})")
        ...
        if fi.has_price_data and fi.estimated_revenue_impact:
            print(f"│  ▸ Revenue at Risk: ${fi.estimated_revenue_impact:,.0f} ({fi.units_at_risk:.0f} units)")
        ...
        if result.severity == "NONE" and result.recommended_quantity > 0 and stock > demand * 3:
            issues.append("⚠ Stock covers 3+ months — why reorder?")
        if result.recommendation_confidence < 0.3 and result.severity in ("CRITICAL", "HIGH"):
            issues.append("⚠ Low confidence on urgent recommendation — risky to trust")
        if fi.has_price_data and fi.estimated_revenue_impact and fi.estimated_revenue_impact > 100000:
...
EDGE_CASES = [
    {
        "name": "Zero Stock",
        "product": ProductData(1, "Zero-Stock-Widget", 1, current_stock=0, avg_monthly_demand=500, unit_price=20.0),
    ...
    },
    {
        ...
        "product": ProductData(2, "Overstocked-Widget", 1, current_stock=60000, avg_monthly_demand=500, unit_price=10.0),
    ...
    {
        "name": "No Supplier",
        "product": ProductData(3, "Orphan-Widget", 1, current_stock=100, avg_monthly_demand=300, unit_price=15.0),
    ...
    },
    {
        ...
        "product": ProductData(4, "No-Forecast-Widget", 1, current_stock=200, avg_monthly_demand=400, unit_price=8.0),
        "forecast": None,
        "supplier": SUPPLIERS["FastParts Inc"],
        "signals": SignalContext(),
    ...
    },
    {
        ...
        "product": ProductData(5, "Calm-Widget", 1, current_stock=500, avg_monthly_demand=600, unit_price=5.0),
    ...
    },
    {
        ...
        "product": ProductData(6, "Risky-Supplier-Widget", 1, current_stock=300, avg_monthly_demand=500, unit_price=12.0),
    ...
    },
    {
        ...
        "product": ProductData(7, "Spike-Widget", 1, current_stock=300, avg_monthly_demand=500, unit_price=25.0),
        "forecast": ForecastData(500, 0.80, "High"),
        "supplier": SUPPLIERS["Acme Components"],
        "signals": SignalContext(active_count=4, critical_count=4, high_severity_signals=[
...
def run_audit_2():
    ...
    for case in EDGE_CASES:
        ...
        if case["name"] == "Zero Stock":
            ...
            if result.recommended_quantity > 0:
                ...
                if result.recommended_quantity > 1000:
                    passed = False
                    notes.append(f"Qty {result.recommended_quantity} too high for overstocked product")

        elif case["name"] == "No Supplier":
            if result.supplier_lead_time_days != 7.0:
                passed = False
                notes.append(f"Expected 7-day default LT, got {result.supplier_lead_time_days}")

        elif case["name"].startswith("No Forecast"):
            if result.confidence != 0.5:
                passed = False
                notes.append(f"Expected 50% confidence fallback, got {result.confidence}")
        ...
        print(f"         │  Severity: {result.severity}  │  Qty: {result.recommended_quantity:,.0f}  │  Days: {result.days_until_stockout}")
        print(f"         │  Confidence: {result.recommendation_confidence:.0%}  │  Expected: {case['expected_quantity']}")
        if notes:
            for n in notes:
                print(f"         │  ⛔ {n}")
        print()
...
def run_audit_3(results: list[ReorderResult]):
    ...
    print("=" * 90)
    print()
    print("Question: If I owned a factory, would these 3 numbers tell me where to focus?")
    print()
    ...
    print("  ├─────────────────────────────────────────────────┤")
    print(f"  │  Forecast Confidence:   {health['components']['forecast_confidence']:5.1f}/100  (30% weight)")
    print(f"  │  Stockout Safety:       {health['components']['stockout_safety']:5.1f}/100  (25% weight)")
    print(f"  │  Signal Health:         {health['components']['signal_health']:5.1f}/100  (25% weight)")
    print(f"  │  Supplier Reliability:  {health['components']['supplier_reliability']:5.1f}/100  (20% weight)")
    print("  ├─────────────────────────────────────────────────┤")
    ...
    priced = [r for r in results if r.financial_impact.has_price_data and r.financial_impact.estimated_revenue_impact]
    priced.sort(key=lambda r: r.financial_impact.estimated_revenue_impact or 0, reverse=True)

    print("  TOP 5 — Revenue at Risk:")
    print(f"  {'Product':<30} {'Risk':>12} {'Qty':>8} {'Severity':<10} {'Confidence':<10}")
    print(f"  {'─' * 30} {'─' * 12} {'─' * 8} {'─' * 10} {'─' * 10}")
    for r in priced[:5]:
        rev = r.financial_impact.estimated_revenue_impact
        print(f"  {r.product_name:<30} ${rev:>10,.0f} {r.recommended_quantity:>7,.0f} {r.severity:<10} {r.recommendation_confidence:.0%}")
    print()
    ...
    for r in urgent[:5]:
        print(f"  {r.product_name:<30} {r.days_until_stockout:>5}d {r.recommended_quantity:>7,.0f} {r.severity:<10} {r.recommended_order_date}")
    print()
    ...
    for r in low_conf[:5]:
        ...
        for reason in r.reasoning:
            if "signal" in reason.lower() or "reliability" in reason.lower() or "No forecast" in reason:
                why = reason[:40]
                break
    ...
    if health["critical_count"] > 0:
        print(f"  ⚠ {health['critical_count']} products are CRITICAL — immediate action required.")
    ...
    if health["score"] >= 80:
        ...
        print("  🔴 Inventory health is CRITICAL. Immediate purchasing action required.")
