...
def calculate_recommendation(
    ...
    if forecast and forecast.forecast_demand > 0:
        ...
        empty_impact = FinancialImpact(
            stockout_risk="NONE", units_at_risk=0.0,
            estimated_revenue_impact=None, has_price_data=False,
        )
    ...
    rec_confidence = calculate_recommendation_confidence(
        ...
        supplier_reliability_pct=(
            supplier.reliability_pct if supplier else None
        ),
        signal_critical_count=signals.critical_count,
    )
    reasoning.append(
        f"Recommendation confidence: {rec_confidence:.0%}"
    )
